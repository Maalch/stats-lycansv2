...
A Vite-based React + TypeScript dashboard for visualizing werewolf game statistics. Uses a unified `gameLog.json` structure with backward-compatible transformation layer. Features three server-side pre-calculation pipelines: Rankings (comparative), Titles (percentile-based), and Achievements (absolute threshold tiers). All three are generated by `scripts/data-sync/` and consumed as static JSON on the client.
...
**Achievement System:** Server-side generation creates `playerAchievements.json` with permanent, level-based unlocks (tiers: bronze/argent/or/lycans)  
**Player Selection:** Dedicated page for player search, selection, Ranking/Achievement display, and interactive navigation
...
npm run generate-achievements   # Achievement generation for main team + copy to public/
```

**Note:** `generate-titles` and team-specific scripts have no npm shortcuts — run directly:
```bash
cd scripts/data-sync && node generate-titles.js [main|discord]
cd scripts/data-sync && node generate-achievements.js [main|discord]
cd scripts/data-sync && node generate-rankings.js [main|discord] [-f]
```
...
**Pre-calculated JSON files:** `playerRankings.json` (Rankings), `playerTitles.json` (Titles), `playerAchievements.json` (Achievements), `playerStatsCache.json` (incremental computation cache — do not edit manually)
...
**Server-Side Generation:** `scripts/data-sync/generate-achievements.js` processes `gameLog.json` (+ `rawBRData.json` for main team) → creates `playerAchievements.json`  
**Client-Side Consumption:** `usePlayerAchievements()` hook loads pre-calculated data  
**Key Difference from Rankings/Titles:** Achievements are permanent absolute thresholds — once unlocked, never revoked. Rankings are comparative, Titles are percentile-based.

### Tier System
```javascript
// 4 tiers, each with up to 3 sub-levels (bronze 1/2/3, argent 1/2/3, or 1/2/3, lycans 1)
ACHIEVEMENT_TIERS = { bronze, argent, or, lycans }
// Defined in: scripts/data-sync/shared/achievementDefinitions.js
```

### Achievement Definition Pattern
```javascript
// scripts/data-sync/shared/achievementDefinitions.js
{
  id: 'achievement-id',
  name: 'Display Name',
  description: 'Flavor text',
  explanation: 'Condition description (French)',
  emoji: '🏆',
...
### Evaluator Pattern
```javascript
// scripts/data-sync/compute/compute-achievements.js
// Standard evaluators receive (playerGames, allGames, playerId, params)
// BR evaluators (BR_EVALUATORS) receive (playerBRGames, brData, params)
// BR games matched by canonical player name — NOT Steam ID (BR data has no IDs)
const EVALUATORS = { campWins, soloWins, deathByType, ... };
...
**Comprehensive voting behavior analysis** with multiple views and metrics.
...
**Animated bar race chart** showing player win rates per calendar month.

- **Component:** `src/components/playerstats/MonthlyRanking/MonthlyRankingChart.tsx`  
- **Eligibility:** Players must have participated in ≥40% of a month's games (`MIN_PARTICIPATION_RATIO = 0.40`)  
- **Animation:** Frame-by-frame playback through each game of the month (configurable speed)  
- **State Persistence:** Month selection + current frame index stored in `navigationState.monthlyRankingState`  
- **Data:** Computed client-side from `useCombinedFilteredRawData()` — no server pre-calculation needed  

## Player Selection System

**Centralized player search and selection interface** with integrated Rankings and Achievements display
...
**Rankings & Titles & Achievements Generation:**
```bash
# Generate Rankings (supports incremental updates via playerStatsCache.json)
...
# Generate achievements (BR achievements auto-skipped for discord - no rawBRData.json)
node generate-achievements.js main
node generate-achievements.js discord
```
