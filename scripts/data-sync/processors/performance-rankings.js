/**
 * Performance Rankings processor - handles camp performance rankings
 */
  ...
  playerSoloPerformance.forEach((data, playerId) => {
    if (data.totalSoloGames >= minGames) {
      ...
      eligiblePlayers.push({
        player: playerId, // Use playerId for consistency with other Rankings
 ...
 */
function createPerformanceRanking(id, title, description, type, rank, value, totalRanked, redirectTo, category = 'performance') {
  return {
    id,
    title,
    description,
...
/**
 * Process camp performance Rankings for a player
 * @param {Array} campStats - Array of player camp statistics
 ...
 */
export function processPerformanceRankings(campStats, gameData, playerId, suffix) {
  const Rankings = [];

  if (!campStats || campStats.length === 0) return Rankings;
  ...
  if (overallPerformanceRank) {
    ...
    Rankings.push(createPerformanceRanking(
