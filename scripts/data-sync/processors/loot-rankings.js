/**
 * Loot Rankings processor - handles loot collection rate rankings
 */
...
/**
 * Process loot collection Rankings for a player
 * @param {Array} gameData - Array of game log entries
 ...
 */
export function processLootRankings(gameData, playerId, suffix) {
  const Rankings = [];

  if (!gameData || gameData.length === 0) return Rankings;

  // Compute loot statistics for all players
  const lootStats = computeLootStats(gameData);

  if (lootStats.length === 0) return Rankings;
