/**
 * Comparison Rankings processor - handles player relationship and matchup stats
 */
...
/**
 * Process face-to-face Rankings for a specific player
 * @param {Array} playerStatsData - Array of player statistics
 * @param {Array} rawGameData - Array of game log entries
...
export function processComparisonRankings(playerStatsData, rawGameData, playerId, suffix) {
  ...
  if (mateRelationships.length > 1) {
    ...
    // Only create "worst mate" Ranking if it's different from best mate and significantly bad
  ...
  if (opponentRelationships.length > 1) {
    ...
    // Only create "worst matchup" Ranking if it's different from best matchup and significantly bad
