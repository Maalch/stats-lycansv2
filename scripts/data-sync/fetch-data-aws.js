...
async function mergeAWSGameLogs(awsGameLogs) {
  console.log('Processing AWS game logs into unified structure...');
  ...
  const unifiedGameLog = {
    ModVersion: "Multiple AWS Versions",
...
async function createDataIndex(awsFilesCount, totalGames) {
  const indexData = {
    ...
    description: "Game logs from AWS S3 bucket only. Updated periodically via GitHub Actions.",
...
async function createPlaceholderFiles() {
  ...
  await saveDataToFile('gameLog-Legacy.json', emptyLegacyGameLog);
...
async function main() {
  console.log('🚀 Starting Lycans AWS-only data sync...');
  ...
  try {
    ...
    console.log('\n🔄 Creating unified dataset from AWS sources...');
    ...
    console.log('\n✅ AWS-only data sync completed successfully!');
    ...
    console.error('❌ AWS data sync failed:', error.message);
    process.exit(1);
  }
}
