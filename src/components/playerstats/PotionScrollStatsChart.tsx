...
import type { CampFilter, EffectFilter, PlayerPotionStats, PlayerScrollUsageStats, PlayerScrollTargetStats, PlayerAssassinStats, PlayerMinusculeStats } from '../../hooks/utils/potionScrollStatsUtils';

type ViewTab = 'potionsGeneral' | 'potionsDetails' | 'scrollsUsed' | 'scrollsReceived';
...
type ChartMinusculeStat = PlayerMinusculeStats & { isHighlightedAddition?: boolean };
...
export function PotionScrollStatsChart() {
  ...
  const validViews: ViewTab[] = ['potionsGeneral', 'potionsDetails', 'scrollsUsed', 'scrollsReceived'];
  ...
  const [selectedView, setSelectedView] = useState<ViewTab>(
    validViews.includes(persistedView) ? persistedView : 'potionsGeneral'
  );
  ...
  const [minMinusculeUses, setMinMinusculeUses] = useState<number>(
    navigationState.potionScrollStatsState?.minMinusculeUses || 5
  );
  ...
  useEffect(() => {
    ...
    if (!current ||
        ...
        current.minMinusculeUses !== minMinusculeUses) {
      updateNavigationState({
        potionScrollStatsState: { minGames, campFilter, effectFilter, selectedView, minAssassinUses, minMinusculeUses }
      });
    }
  }, [minGames, campFilter, effectFilter, selectedView, minAssassinUses, minMinusculeUses, updateNavigationState]);
  ...
  // ── Minuscule kill rate chart data ────────────────────────────────────────
  const { minusculeKillRateData, highlightedAddedMinuscule } = useMemo(() => {
    if (!data?.minusculeStats) return { minusculeKillRateData: [] as ChartMinusculeStat[], highlightedAddedMinuscule: false };
    const eligible = data.minusculeStats.filter(p => p.minusculePotionsUsed >= minMinusculeUses);
    const sorted = [...eligible].sort((a, b) => b.killRatio - a.killRatio).slice(0, CHART_LIMITS.TOP_20);
    const inTop = settings.highlightedPlayer && sorted.some(p => p.player === settings.highlightedPlayer);
    let final: ChartMinusculeStat[] = [...sorted];
    let added = false;
    if (settings.highlightedPlayer && !inTop) {
      const hp = data.minusculeStats.find(p => p.player === settings.highlightedPlayer);
      if (hp) { final.push({ ...hp, isHighlightedAddition: true }); added = true; }
    }
    return { minusculeKillRateData: final, highlightedAddedMinuscule: added };
  }, [data, minMinusculeUses, settings.highlightedPlayer]);
  ...
  const eligibleCount = (selectedView === 'potionsGeneral' || selectedView === 'potionsDetails')
  ...
  function minusculeTooltip({ active, payload }: { active?: boolean; payload?: ReadonlyArray<{ payload: ChartMinusculeStat }> }) {
    if (!active || !payload?.length) return null;
    const d = payload[0].payload;
    return (
      <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: '4px', padding: '8px', fontSize: '0.85rem' }}>
        ...
        <div>Parties jouées: {d.gamesPlayed}</div>
        <div style={{ marginTop: '4px', paddingTop: '4px', borderTop: '1px solid var(--border-color)' }}>
          <strong>Potion Minuscule:</strong>
        </div>
        <div>Potions utilisées: {d.minusculePotionsUsed}</div>
        <div>Morts écrasé: {d.crushedDeaths}</div>
        <div>Taux: {d.killRatio.toFixed(1)}%</div>
        {d.isHighlightedAddition && (
          <div style={{ marginTop: '4px', paddingTop: '4px', borderTop: '1px solid var(--accent-primary)', color: 'var(--accent-primary)', fontStyle: 'italic' }}>
            🎯 Affiché via sélection personnelle
            {d.minusculePotionsUsed < minMinusculeUses && ` (< ${minMinusculeUses} utilisation${minMinusculeUses > 1 ? 's' : ''})`}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="lycans-players-stats">
      ...
      <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '2rem' }}>
        <button type="button" onClick={() => setSelectedView('potionsGeneral')} style={viewTabStyle(selectedView === 'potionsGeneral')}>
          🧪 Potions - Général
        </button>
        <button type="button" onClick={() => setSelectedView('potionsDetails')} style={viewTabStyle(selectedView === 'potionsDetails')}>
          🔬 Potions - Détails
        </button>
      ...
      {/* Potions - Général view */}
      {selectedView === 'potionsGeneral' && (
        renderChartPair(
      ...
      {/* Potions - Détails view */}
      {selectedView === 'potionsDetails' && (() => {
        const eligibleAssassinCount = data.assassinStats.filter(p => p.assassinPotionsUsed >= minAssassinUses).length;
        const eligibleMinusculeCount = data.minusculeStats.filter(p => p.minusculePotionsUsed >= minMinusculeUses).length;
        return (
          <>
            <div className="lycans-graphique-section">
              <div>
            ...
            <div className="lycans-graphique-section">
              <div>
                <h3>Taux de Kill - Potion Minuscule 🐜</h3>
                {renderHighlightedNote(highlightedAddedMinuscule)}
              </div>
              <div className="lycans-winrate-controls" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
                <label htmlFor="camp-minuscule" style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Camp:</label>
                <select id="camp-minuscule" value={campFilter} onChange={(e) => setCampFilter(e.target.value as CampFilter)}
                  style={{ background: 'var(--bg-tertiary)', color: 'var(--text-primary)', border: '1px solid var(--border-color)', borderRadius: '4px', padding: '0.25rem 0.5rem', fontSize: '0.9rem' }}>
                  <option value="all">Tous les camps</option>
                  <option value="villageois">Camp Villageois</option>
                  <option value="loup">Camp Loup</option>
                ...
                <label htmlFor="min-minuscule" style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginLeft: '1rem' }}>Min. utilisations:</label>
                <select id="min-minuscule" value={minMinusculeUses} onChange={(e) => setMinMinusculeUses(Number(e.target.value))}
                  style={{ background: 'var(--bg-tertiary)', color: 'var(--text-primary)', border: '1px solid var(--border-color)', borderRadius: '4px', padding: '0.25rem 0.5rem', fontSize: '0.9rem' }}>
                  {[1, 5, 10, 25, 50].map(option => <option key={option} value={option}>{option}</option>)}
                </select>
              </div>
              <FullscreenChart title="Taux de Kill - Potion Minuscule">
                <div style={{ height: 400 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={minusculeKillRateData} margin={{ top: 20, right: 30, left: 20, bottom: 80 }}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="player" angle={-45} textAnchor="end" height={80} interval={0} tick={xAxisTick} />
                      <YAxis
                        label={{ value: 'Morts écrasé / Potions (%)', angle: 270, position: 'left', offset: 15, style: { textAnchor: 'middle' } }}
                        tickFormatter={(v: number) => `${v}%`}
                        domain={[0, 100]}
                      />
                      <Tooltip content={minusculeTooltip} />
                      {renderBar('killRatio')}
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              ...
              <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', textAlign: 'center', marginTop: '0.5rem' }}>
                Top {minusculeKillRateData.length} des joueurs (sur {eligibleMinusculeCount} ayant utilisé Minuscule au moins {minMinusculeUses} fois)
              </p>
              <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', textAlign: 'center', fontStyle: 'italic', marginTop: '0.25rem' }}>
                Ratio morts écrasé / potions Minuscule utilisées · Calculé sur {data.gamesWithActionData} parties avec données d&apos;actions
              </p>
            </div>
          </>
        );
